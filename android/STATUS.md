...
Reviewed 2026-09-26. Release: **0.8.12**, versionCode 22 (unified with Windows's version number as of this release). See the [platform comparison](../PROJECT_STATUS.md).

This release packages everything that had accumulated in the working tree since 0.8.7: the people-screen side menu and the `Show offline users` filter; Delete conversation / Delete app data (mirrors Windows); and a contact-forget notice plus group-leave + owner re-invite (mirrors the same Windows 0.8.12 addition — see [Windows status](../windows/STATUS.md)).
...
- Package: `D:/LAN-Messenger/outputs/LanMessenger-0.8.12.apk`; install over the existing app without uninstalling. Test outputs: `D:/LAN-Messenger/outputs/.build/release-tests-087`.
- Last Android code commit: `7d9d99c`; the people side menu, delete-conversation/delete-app-data, and forget-notice/group-re-invite work committed locally on `master` as part of the 0.8.12 release, not pushed.
