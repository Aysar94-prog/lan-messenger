...
For internal regression harnesses, an 8-field MSG is accepted as a direct text message. Production 0.4 sends all 12 fields. A MSG with a filename is rejected before reading a body. Attachments use OFFER with the same fields (13 for signed groups), without any body. Size is a signed 64-bit count bounded to 1 TiB; normal sending remains capped at 1 GiB. SHA-256 is mandatory even for empty files. Plain text has empty filename/hash and size zero. Validate names, metadata and required group signatures before saving and ACKing. ACK acknowledges the offer, not possession of file data. File names never determine storage paths.
...
## Streamed attachments up to 1 GiB (0.7.4)

An attachment's *plaintext content* is never fully buffered in memory on either the local-storage or the network path: every read/write moves it through fixed-size chunks (3 MiB as of 0.7.5, raised from the initial 1 MiB), from disk to the encrypted store, from the store to the network, and the reverse on receipt. This is what makes a 1 GiB attachment (`MaxFileSize`/`MAX_FILE_SIZE`) practical without risking an out-of-memory condition, particularly on Android. Validation is still end-to-end only, not per chunk: each chunk is written/sent as soon as it's read, with no integrity check of its own — the single SHA-256 hash covering the whole reassembled content (checked once, after the full transfer completes) is what catches corruption, exactly as before this file's chunk size changed.

This required a new on-disk attachment format, since the previous one (`protector.Protect`/`protect` over the whole file at once) needs the entire plaintext in memory to encrypt in a single call. The new format: a 7-byte ASCII magic (`LMATCS1`), a 4-byte big-endian length prefix, that many bytes of `protector.Protect(random 32-byte key ‖ random 16-byte IV)`, then the file content encrypted with AES-256-CBC under that key/IV, streamed in chunks. This is confidentiality-only at rest — no per-chunk authentication tag — deliberately trading the DPAPI/Keystore-backed whole-file encryption's implicit authentication for the ability to stream at all; corruption or tampering is caught the same way it always was, by comparing the plaintext's SHA-256 hash against the value already carried in the message record. A device still holding attachments saved before 0.7.4 (no magic prefix, or too short to contain one) reads them via the old single-shot `Unprotect` path automatically — both formats coexist indefinitely, and nothing is re-encrypted or migrated on upgrade.

The wire format is unaffected: the same MSG/RELAY frame header, followed by exactly as many raw bytes as declared, exactly as before — only *how many separate `read`/`write` calls* move those bytes changed (many small ones instead of one large one), which TCP does not distinguish on the receiving end. A device on an older version interoperates normally; it is only bound by its own (lower) `MaxFileSize`.

Java's `Socket` has no native write-timeout (unlike the read timeout set once at connect time via `setSoTimeout`) — a peer that stops reading could otherwise block a sender's thread forever, and with only 2–6 threads shared across all of a process's connections, just one or two such stalls could starve every other peer's delivery indefinitely, with no exception ever thrown to explain why. `NetworkTimeoutOutputStream` closes the socket via a watchdog if a single chunk write doesn't complete within 45 seconds. The C# side already had equivalent protection (`NetworkTimeoutStream`, wrapping both directions with a per-call `CancellationToken`).
...
- Server requires a retained, unexpired offer, complete data/original source, and its verified direct recipient or a verified group member. Two serving streams maximum; excess requests receive `LM4<TAB>BUSY`. Missing/unauthorized data receives UNAVAILABLE or connection closure.
