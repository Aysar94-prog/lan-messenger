...
An attachment's *plaintext content* is never fully buffered in memory on either the local-storage or the network path: every read/write moves it through fixed-size chunks (1 MiB), from disk to the encrypted store, from the store to the network, and the reverse on receipt. This is what makes a 1 GiB attachment (`MaxFileSize`/`MAX_FILE_SIZE`) practical without risking an out-of-memory condition, particularly on Android.
