...
- **Attachment limit raised from 10 MiB to 200 MiB.** Network timeouts now scale with file size so large transfers get enough time to complete on ordinary Wi-Fi. This is a straightforward cap increase, not a rework of the transfer itself: there's still no progress bar, pause/resume, or streaming to disk — a big file reads fully into memory before sending, so very large or repeated transfers can be slow or memory-heavy, especially on a phone.
